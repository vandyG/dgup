...
def test_exposed_objects(modulelevel_internal_objects: list[griffe.Object | griffe.Alias]) -> None:
    """All public objects in the internal API are exposed under `dgup`."""
    not_exposed = [
        obj.path
        for obj in modulelevel_internal_objects
        if obj.name not in dgup.__all__ or not hasattr(dgup, obj.name)
    ]
...
def test_inventory_matches_api(
    ...
    public_api_paths.add("dgup")
    for item in inventory.values():
        if (
            item.domain == "py"
            and "(" not in item.name
            and (item.name == "dgup" or item.name.startswith("dgup."))
        ):
